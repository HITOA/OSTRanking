...
CREATE TABLE IF NOT EXISTS shows(id INT NOT NULL PRIMARY KEY, native NVARCHAR(512), preferred NVARCHAR(512), english NVARCHAR(512), medium VARCHAR(512), large VARCHAR(512));
...
CREATE TABLE IF NOT EXISTS author_ost(author_id INT NOT NULL, ost_id INT NOT NULL, role VARCHAR(32) NOT NULL, PRIMARY KEY (author_id, ost_id), FOREIGN KEY (author_id) REFERENCES authors(id), FOREIGN KEY (ost_id) REFERENCES osts(id));
CREATE TABLE IF NOT EXISTS show_ost(show_id INT NOT NULL, ost_id INT NOT NULL, type SMALLINT NOT NULL, num SMALLINT NOT NULL, PRIMARY KEY (show_id, ost_id), FOREIGN KEY (show_id) REFERENCES shows(id), FOREIGN KEY (ost_id) REFERENCES osts(id));
CREATE TABLE IF NOT EXISTS tag_ost(tag_id INT NOT NULL, ost_id INT NOT NULL, PRIMARY KEY (tag_id, ost_id), FOREIGN KEY (tag_id) REFERENCES tags(id), FOREIGN KEY (ost_id) REFERENCES osts(id));
